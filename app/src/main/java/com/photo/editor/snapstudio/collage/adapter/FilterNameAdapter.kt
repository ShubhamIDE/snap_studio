...
class FilterNameAdapter(context: Context,filterList:Array<String>) : Adapter<FilterNameAdapter.FilterNameHolder>() {
    ...
    override fun onBindViewHolder(holder: FilterNameHolder, position: Int) {
        ...
        val isDarkTheme = mContext.getSharedPreferences(
            mContext.resources.getString(R.string.app_name),
            AppCompatActivity.MODE_PRIVATE
        ).getBoolean("isDarkTheme", false)

        if (isDarkTheme) {
            holder.txt_filter_tab.setTextColor(mContext.resources.getColor(R.color.white))
        } else {
            holder.txt_filter_tab.setTextColor(mContext.resources.getColor(R.color.blacktwo))
        }
