package com.photo.editor.snapstudio.collage.frame.utils
...
object PhotoUtils {
    ...
    fun fillblacktwoToImage(bitmap: Bitmap, color: Int): Bitmap {
    ...
    fun loadImageWithGlide(context: Context, imageView: ImageView, uri: String?) {
        if (uri != null && uri.length > 1) {
            if (uri.startsWith("http://") || uri.startsWith("https://")) {
                Glide.with(context).load(uri).diskCacheStrategy(DiskCacheStrategy.RESOURCE)
                    .into(imageView)
            } else if (uri.startsWith(DRAWABLE_PREFIX)) {
                try {
                    val id = Integer.parseInt(uri.substring(DRAWABLE_PREFIX.length))
                ...
                val file = uri.substring(ASSET_PREFIX.length)
                Glide.with(context).load(Uri.parse("file:///android_asset/$file"))
                    .signature(ObjectKey(System.currentTimeMillis().toString())).into(imageView)
            } else {
